...
Note that $\hat{Z}_1 \thicksim P_{Z + \Delta X}$

\begin{figure}[!h]
    \centering
    \begin{tikzpicture}[scale=3]
        \node[left] (O) at (0,0) {$P_Z$};
        \draw[semithick, fill=black] (0,0) circle (0.02);
        %Draw the Circle around it all
        \draw[thick] (0,0) circle (1);
        %Draw lines to the top
        % \foreach \x in {-4,...,4}
        %   \draw[red,very thick] (\x/8,0) -- +(0,{sqrt(1-\x/8*\x/8)});
        \path (0, 0) -- (0, 1) node[above] {$P_{Z + \alpha X}$};
        \draw[thick, <->] (0, .05) -- node[right] {$R(\alpha)$} (0, .95);
        \draw[semithick, fill=black] (0,1) circle (0.02);
        % p_1
        \draw[semithick, fill=black] (.5, -.5) circle (0.02);
      \end{tikzpicture}
      \caption{Proof idea}
\end{figure}

We begin by proving two lemmas that we will use for the consistency proof. 

\begin{lemma} Given $\alpha, \delta > 0$, random variables $N$ and $X$ s.t. $\alpha X + N$, where $X \bigCI N$, 
