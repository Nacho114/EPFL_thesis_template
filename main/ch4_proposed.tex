...
taking measurements, the additive noise of our devices tends to be independent.
...
follow the same distribution (due to the non-linearlities introduced by f and the additivity of the noise) and 
...
Say that we partition $\mathcal{D}$ into disjoint sets $\mathcal{D}_1, ..., \mathcal{D}_k$; then these 
...
with only two clusters; but what if we have a lot of data? As we will see, we observe experimentally that if we 
...
\begin{algorithm}[H]
    ...
    \begin{enumerate}
        ...
        $\mathbf{Z} := \{ \mathbf{\hat{e}_{Y}}(i) \}_{i \in [k]} $
        ...
        \begin{itemize}
            \item[--] $\hat{C}_{X \rightarrow Y}: = \hat{C}( \mathbf{Z} )$ 
...
general set up should follow with little effort from our proof. 
...
The residual method is the precursor of the twin test. Here to we try to exploit knowledge of the noise
directly; the key difference is that here we assume that we know $P_Z$, i.e. the distribution of the 
additive noise.
...
\subsection{Bounding the false false positive}
