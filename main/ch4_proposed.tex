...
Note that as $k$ increases so does $C^*$, since we are growing the space to optimse for $C^*$; this is 
...
Where $P^*_Z$ is the quantised version of $P_Z$.
...
Both of which go to zero for any $\alpha > 0$ assuming that our regression is \textit{faithfull}.
