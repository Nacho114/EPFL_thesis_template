...
partitions need to satisfy three requirements:
...
faithful, we can chose any $\alpha$ such that
...
$\hat{f}_X$ for the reverse model. We then compute the residuals
...
The idea is then to first discretize\footnote{We do so in a naive manner we split
...
Recall that what is left to bound is the following quantity, $\mathbb{P}\left[W \leq \tau \right]$; 
...
In essence, we are asking: "what is the chance that the realization of $\tilde{B}$ – which is the 
...
We note that to get the best bound we may maximizes the r.h.s. w.r.t. $\tau$.