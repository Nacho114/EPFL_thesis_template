...
application, these will be quantised versions of these distributions. The following lemma tells us 
that this is not a problem, the $l_1$ distance between the quantised version cannot be bigger than 
that of their continuos counter parts. The only requirement is that we fix the quantisation scheme
beforehand and use the same one for both.

\begin{lemma}
    Let $P$ and $Q$ be two continuos distribtions, then let $P^*$, $Q^*$ resp. be discretized versions. Then if 
...
\begin{proof}
    ...
    Given continuos distribtions $P$ and $Q$, we form their quantised counter parts as
    follows:

    $$
        P^*(k) := \sum _{i \in \Z} \; \int_{I_i} p(t) dt \; \Ind_{k \in I_i}
        , \quad   Q^*(k) := \sum _{i \in \Z} \; \int_{I_i} q(t) dt \; \Ind_{k \in I_i}
    $$
    ...
    \begin{align*}
        ...
        &\leq \sum _{k \in \Z} \sum _{i \in \Z} \; \left| \int_{I_i} \left(p(t) - q(t)\right) dt \right|  \; \Ind_{k \in I_i} \\
        &\leq \sum _{k \in \Z} \sum _{i \in \Z} \;  \int_{I_i} \left|p(t) - q(t)\right| dt  \; \Ind_{k \in I_i} \\
        &= \sum _{i \in \Z} \;  \int_{I_i} \left|p(t) - q(t)\right| dt  \; \sum _{k \in \Z} \Ind_{k \in I_i} \\
        &= \sum _{i \in \Z} \;  \int_{I_i} \left|p(t) - q(t)\right| dt \\
        &= \int_{\R} \left|p(t) - q(t)\right| dt \\
        &= \norm{P - Q}_1^2
    \end{align*}

    Note that $\sum _{k \in \Z} \Ind_{k \in I_i} = 1$ for any $i$ and $k$; since $I_i$ is a disjoint
    partition $k$ will always belong to one of them, and one alone. 
\end{proof}
...
Thus if we are given some $\epsilon > 0$, we first pick $\delta = \frac{\epsilon}{4}$, and $\alpha > \frac{\epsilon}{4C}$. 
