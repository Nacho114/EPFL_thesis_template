...
The inequality follows from the convexity of $f$, this not only tells us that 
$\infdiv{P}{Q} \geq 0$. From the definition it is clear that $\infdiv{P}{P} = 0$;
one can also easily show that $\infdiv{P}{Q} = 0$ for all $P \neq Q$ iff 
$f(x) = c(x - 1)$ for some $c$. For all other we have $\infdiv{P}{Q} > 0$ if $P \neq Q$ not exactyl trrru
